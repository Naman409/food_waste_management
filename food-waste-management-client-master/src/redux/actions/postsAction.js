...
export const LoadPosts = (onSuccess) => {
    return async (dispatch, getState) => {
        try {
            const posts = await axios.get(`${server}/api/post/allposts`);
        ...
        try {
            ...
            images.map(async (image) => {
                ...
                if (imgUrls.length === images.length) {
                    ...
                    const upload = await axios.post(`${server}/api/post/addpost`, sdata, {
                        headers: {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            images.map(async (image) => {
                ...
                if (imgUrls.length === images.length) {
                    sdata['images'] = imgUrls;
                    const upload = await axios.put(`${server}/api/post/${id}`, sdata, {
                        headers: {
                            'authentication': localStorage.getItem('token')
                        }
                    });
...
export const DeletePost = (id) => {
    return async (dispatch, getState) => {
        ...
        try {
            const status = await axios.delete(`${server}/api/post/${id}`, {
                headers: {
                    'authentication': localStorage.getItem('token')
...
export const upVote = (postId, currentUserId) => {
    return async (dispatch, getState) => {
        try {
            const vpvoted = await axios.post(`${server}/api/post/upvote`, { id: postId }, {
                headers: {
                    'authentication': localStorage.getItem('token')
